                    ...
                    <td class="text-end">
                        <x-form action="{{ route('categories.destroy', $category) }}" method="post">
                            <a class="btn btn-primary" href="{{ route('categories.edit', $category) }}">
                                <i class="bi bi-pencil-square"></i> <span class="d-none d-md-inline">{{ __('Edit') }}</span></a>
                            @csrf
                            @method('delete')
                            <x-form-submit type="submit" class="btn btn-danger delsoft">
                                <i class="bi bi-trash3-fill"></i> <span class="d-none d-md-inline">{{ __('Delete') }}</span></x-form-submit>
                        </x-form>
                    </td>
                </tr>
            @endforeach
